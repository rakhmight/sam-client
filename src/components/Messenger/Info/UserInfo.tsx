...
export default function(){
    ...
    const handleInfoPanel = (param:string)=>{
        changeInfoPanel({ mode: 'files', des: param })
    ...
    return (
        <div className="w-full h-full bg-slate-100 py-2.5 flex flex-col items-start gap-3">
            ...
            <div className="media w-full">
                <div className='w-full flex flex-row gap-5 items-center px-4 py-1.5 hover:bg-slate-200 hover:cursor-pointer' onClick={()=> handleInfoPanel('files') }>
                    <div><Icon as={ImFileEmpty} fontSize='18px' color='var(--special-color)' /></div>
                    <div><Text fontSize='sm'>1 files</Text></div>
                </div>
                <div className='w-full flex flex-row gap-5 items-center px-4 py-1.5 hover:bg-slate-200 hover:cursor-pointer' onClick={()=> handleInfoPanel('images') }>
                    <div><Icon as={ImFilePicture} fontSize='18px' color='var(--special-color)' /></div>
                    <div><Text fontSize='sm'>2 images</Text></div>
                </div>
                <div className='w-full flex flex-row gap-5 items-center px-4 py-1.5 hover:bg-slate-200 hover:cursor-pointer' onClick={()=> handleInfoPanel('videos') }>
                    <div><Icon as={ImFilePlay} fontSize='18px' color='var(--special-color)' /></div>
                    <div><Text fontSize='sm'>1 videos</Text></div>
                </div>
                <div className='w-full flex flex-row gap-5 items-center px-4 py-1.5 hover:bg-slate-200 hover:cursor-pointer' onClick={()=> handleInfoPanel('audios') }>
                    <div><Icon as={ImFileMusic} fontSize='18px' color='var(--special-color)' /></div>
                    <div><Text fontSize='sm'>1 audio files</Text></div>
                </div>
            </div>
        </div>
    )
